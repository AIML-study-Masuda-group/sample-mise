...
2. `uv lock`を実行します。
3. `uv sync --frozen`を実行します（グローバル設定では `mise run sync` で実行可能）。
4. `uv run python app/main.py`で実行。[Python内の参考](/app/main.py)。

### グローバルツールとプロジェクト依存の使い分け

mise/config.toml では、Python 開発ツール（ruff, mypy 等）を2つの方法で管理できます：

1. **グローバルツール**（`uv tool install` 経由）
   - 複数プロジェクトで共通して使うツール
   - `mise run install-global-tools` でインストール
   - システム全体で利用可能

2. **プロジェクト依存**（`pyproject.toml` の `[tool.uv.dev-dependencies]` 経由）
   - プロジェクト固有のバージョンが必要な場合
   - `uv sync --extra dev` または `mise run sync-dev` でインストール
   - プロジェクトの .venv 内に配置

**推奨:**
- ruff, mypy 等の基本ツールはグローバルにインストール
- プロジェクト固有のバージョンが必要な場合のみ pyproject.toml で管理
